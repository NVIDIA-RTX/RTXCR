...
#ifndef _RTXCR_BSDF_HLSLI_
#define _RTXCR_BSDF_HLSLI_
...
#define ONE_OVER_TWO_PI (1.0f / RTXCR_TWO_PI)
#endif
...
float evalFresnelSchlick(float f0, float f90, float NdotS)
{
    return f0 + (f90 - f0) * pow(1.0f - NdotS, 5.0f);
}
...
float3 evalLambertianBRDF(float3 N, float3 L, MaterialProperties material) {
	// Prepare data needed for BRDF evaluation - unpack material properties and evaluate commonly used terms (e.g. Fresnel, NdotL, ...)

	const float3 dummyV = 0.0f;
	const BrdfData data = prepareBRDFData(N, L, dummyV, material);
	return evalLambertian(data);
}

float3 evalLambertianBRDF(float3 N, float3 L, float3 V, MaterialProperties material) {
	// Prepare data needed for BRDF evaluation - unpack material properties and evaluate commonly used terms (e.g. Fresnel, NdotL, ...)
	const BrdfData data = prepareBRDFData(N, L, V, material);
	return evalLambertian(data);
}

float3 evalDiffuse(float3 N, float3 L, float3 V, MaterialProperties material) {
	// Prepare data needed for BRDF evaluation - unpack material properties and evaluate commonly used terms (e.g. Fresnel, NdotL, ...)
	BrdfData data = prepareBRDFData(N, L, V, material);
	return evalDiffuse(data);
}

float3 evalSpecular(float3 N, float3 L, float3 V, MaterialProperties material) {
	// Prepare data needed for BRDF evaluation - unpack material properties and evaluate commonly used terms (e.g. Fresnel, NdotL, ...)
	BrdfData data = prepareBRDFData(N, L, V, material);
	return evalSpecular(data);
}
...
bool evalIndirectCombinedBRDF(float2 u, float3 shadingNormal, float3 geometryNormal, float3 V, MaterialProperties material, const int brdfType, const float refractiveIndex, OUT_PARAMETER(float3) rayDirection, OUT_PARAMETER(float3) sampleWeight, OUT_PARAMETER(float) pdf) {
	// Ignore incident ray coming from "below" the hemisphere
	rayDirection = float3(0.0f, 0.0f, 0.0f);
	pdf = 0.0f;
	sampleWeight = float3(0.0f, 0.0f, 0.0f);
